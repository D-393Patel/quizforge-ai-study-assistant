...
type Props = { quiz: Quiz; difficulty: string; mock?: boolean; fallbackUsed?: boolean; attempt?: number; initialAnswers?: AnswerMap; initialChallenges?: ChallengeMap; onProgress: (answers: AnswerMap, challenges: ChallengeMap) => void; onSubmit: (answers: AnswerMap, challenges: ChallengeMap) => void; onExit: () => void };

export function QuizPlayer({ quiz, difficulty, mock = false, fallbackUsed = false, attempt = 1, initialAnswers = {}, initialChallenges = {}, onProgress, onSubmit, onExit }: Props) {
  ...
  return <main className="quiz-shell">
    ...
    <section className="quiz-card" aria-labelledby="quiz-title">
      ...
      <div className="quality-strip"><span><Check size={14} /> {quiz.questions.length} questions</span><span><Check size={14} /> Response validated</span><span><Check size={14} /> {difficulty} difficulty</span>{mock && <span className="demo-badge">Demo data · not generated from your notes</span>}{fallbackUsed && <span className="fallback-badge">Generated with backup provider</span>}</div>
